function pianoroll2(handle,nmat,varargin)
% Plot pianoroll notation of NMAT
% pianoroll2(handle,nmat,varargin)
% Plots pianoroll notation of NMAT and takes several optional arguments
% that affect the information displayed.
% Modificado para que pueda ser visualizado en un grafico de Matlab App
% designer. Por lo que se ha utilizado handle para asignar UIAxes. 
%
% Input arguments: 
%	NMAT = notematrix
%   handle = UIAxes (App Designer)
...
% 		p=pitch2(n);
% 		proll(handle,n,nameformat,timeformat,velformat,cols(i,:),'hold');
% 		text(n(1),p(1)+1.5,['ch' num2str(i)],'Color', cols(i,:),'Fontsize',11);
% 	end
% else
% 		proll(handle,nmat,nameformat,timeformat,velformat,color,status);
% end

% function proll(handle,nmat,nameformat,timeformat,velformat,color,status)
...
if strcmp(timeformat,'beat') == 1
	...
	p=pitch2(nmat);
	minimi = min(pitch2(nmat));
	maksimi = max(pitch2(nmat));
...
for k=1:length(on)
   ...
   h=plot(handle,[on(k) off(k) off(k) on(k) on(k)],[p(k)-0.5 p(k)-0.5 p(k)+0.5 p(k)+0.5 p(k)-0.5 ],color); end % default 'b'
...
if strcmp(status,'hold')==1
	...
	axis(handle,[0 totaldur minimi-0 maksimi+0]);
else
	axis(handle,[0 totaldur minimi-2 maksimi+2]);
end
...
% 			xlabel(handle,'Time in seconds','FontSize',13);
% 		else
% 			xlabel(handle,'Time in beats','FontSize',13);
% 		end
% 
% 		ylabel(handle,'Pitch','FontSize',13);
% 				set(gca,'Xgrid','on');
...
%%% MARK ALL C notes
% 	line(handle,[0 totaldur],[48 48],'LineWidth',1,'LineStyle','-','Color',[0.35 0.35 0.35])
% 	line(handle,[0 totaldur],[60 60],'LineWidth',1,'LineStyle','-','Color',[0.35 0.35 0.35])
% 	line(handle,[0 totaldur],[72 72],'LineWidth',1,'LineStyle','-','Color',[0.35 0.35 0.35])
% 	line(handle,[0 totaldur],[84 84],'LineWidth',1,'LineStyle','-','Color',[0.35 0.35 0.35])
% 	line(handle,[0 totaldur],[96 96],'LineWidth',1,'LineStyle','-','Color',[0.35 0.35 0.35])
